...
# al. (2013)
setwd('C:/Users/Usuario/OneDrive - Universidad de Castilla-La Mancha/00 CURSOS/MasterIREC2023/02 Modelos de captura recaptura')
library(lattice)
library(coda)
...
for(i in seq_along(zNodes)) conf$addSampler(target = zNodes[i], type = custom_zs_sampler, control = list(sNodes = sNodePairs[[i]], birthProb = 0.5, deathProb = 0.5 ))
for(i in seq_along(sNodePairs)) conf$addSampler(target = sNodePairs[[i]], type = custom_s_with_indicator_sampler, control = list(scaleFactor = 0.5, scaleNode = 'sigma', indicatorNode = zNodes[i]))
